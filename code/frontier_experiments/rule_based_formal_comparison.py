...
def method_params(config: FormalComparisonConfig) -> tuple[tuple[RuleMethod, int | None, int | None], ...]:
    return (
        ("NP", None, None),
...
def run_instance(
    ...
    for method, threshold, max_platoon_size in method_params(config):
        ...
        if method == "NP" and np_average_for_gap is None and schedule.status == "OPTIMAL":
            np_average_for_gap = schedule.objective_average_delay
    ...
    objective_by_method = {
        str(row["method"]): row.get("objective")
        for row in rows
        if row.get("status") == "OPTIMAL" and row.get("objective") is not None
    }
    relation_holds = None
    if {"NP", "PP", "CHP"}.issubset(objective_by_method):
        relation_holds = (
            float(objective_by_method["NP"])
            <= float(objective_by_method["PP"]) + 1e-7
            <= float(objective_by_method["CHP"]) + 1e-7
        )
    for row in rows:
        row["np_pp_chp_delay_order_holds"] = relation_holds
    return rows
...
def summarize(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for row in rows:
        groups.setdefault((int(row["N"]), float(row["arrival_rate"]), str(row["method"])), []).append(row)
    summary: list[dict[str, object]] = []
    for (n_value, arrival_rate, method), group in sorted(groups.items()):
        summary.append(
            {
                "N": n_value,
                "arrival_rate": arrival_rate,
                ...
                "actual_gap_case_count": len(numeric_values(group, "actual_optimality_gap")),
...
def run(config: FormalComparisonConfig) -> dict[str, object]:
    ...
    relation_rows = [row for row in rows if row.get("method") == "NP" and row.get("np_pp_chp_delay_order_holds") is not None]
