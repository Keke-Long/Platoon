\section*{Candidate Proof Development}

This note records the full candidate proof separately from the formal manuscript. Symbols follow \texttt{preamble.tex}. The result must not be cited as established until exhaustive verification is complete.
...
Using the positive part provides a nonnegative per-repair bound. Repeating the repair across all platoon internal links and applying a telescoping sum yields the candidate sequence-specific bound in Eq.~\eqref{eq:candidate-sequence-bound}. Replacing every \(M_e\) with \(N\) yields Eq.~\eqref{eq:candidate-partition-bound}.

\subsection*{Unresolved proof checks}

\begin{enumerate}
  \item Verify that the proposed repair ordering always preserves all previously established platoon adjacencies.
  \item Verify the suffix inequality for every combination of the approaches of \(b_m\), \(a'\), and \(x\).
  ...
  \item Determine whether the refined parameter-based zero-loss condition is genuinely valid or only an artifact of the candidate bound.
