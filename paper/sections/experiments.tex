...
\begin{figure}[t]
  ...
  \caption{Method-level mean vehicle delay of NP, CHP, and PP as the platooning threshold or vehicle arrival rate increases, using checked exact-delay rows. The left panel varies \(\delta\), and the right panel varies \(\lambda\). For this presentation figure, PP is averaged over \(P_{\max}\) to show its aggregate delay position relative to NP and CHP; light shaded bands show the interquartile range behind each method-level mean.}
  \label{fig:pp-delay}
\end{figure}
