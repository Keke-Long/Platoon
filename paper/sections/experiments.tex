...
The \(N=20\) formal run is complete. The larger-scale \(N=40,60,80\) results will be added after those runs finish. For \(N=20\), all 960 PP rows are bound-checkable, giving 100\% bound-check coverage. No bound violation is observed. The mean actual optimality gap is \(0.331\), and the median actual gap is \(0.000\). The corresponding mean rule-level upper bound is \(52.200\), and the median upper bound is \(45.275\). The minimum observed bound slack is \(6.950\). These results support the intended interpretation of the rule-level bound: it is valid on every checked case in the completed \(N=20\) domain, but it is conservative and should not be interpreted as a point prediction of the realized gap.
...
The \(N=20\) formal comparison is complete. Larger-scale results for \(N=40,60,80\) will be added after those runs finish. Among the 960 checked PP rows, there are zero delay-order failures, meaning every checked case satisfies \(D_{\mathrm{NP}}\le D_{\mathrm{PP}}\le D_{\mathrm{CHP}}\). The mean delay is \(8.531\) for NP, \(8.862\) for PP, and \(8.977\) for CHP. The PP solutions use an average of \(7.786\) platoons and \(26.416\) ordering variables, corresponding to a mean dimension-reduction ratio of 82.4\%.

The completed \(N=20\) results show the expected qualitative behavior. Larger \(\delta\) generally increases delay while reducing the number of platoons. The effect of \(P_{\max}\) is strongest at small caps and then saturates at larger values for \(N=20\), where the traffic instances are not large enough for the biggest platoon-size caps to remain active in many cases. The fixed-budget trajectory figure is retained only as a provisional pipeline check from the \(N=20\) run; the final manuscript figure will be selected after the \(N=40\) run unless no more representative instance is available.
...
\begin{figure}[t]
  ...
  \caption{Provisional quality of Gurobi solutions as a function of wall-clock time for NP, CHP, and representative PP parameter settings. The current \(N=20\) trajectory is retained only as a pipeline check. After the \(N=40\) run is complete, the final figure will use a representative instance where NP has multiple incumbent updates or reaches the time limit, PP has at least two callback points, and all methods use the same instance, unless no better instance exists.}
