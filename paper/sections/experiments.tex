...
\begin{figure}[t]
  ...
  \caption{Three-dimensional bound validation for the completed \(N=20\) and \(N=40\) runs. The horizontal axes are the platooning threshold \(\delta\) and maximum platoon size \(P_{\max}\), and the vertical axis shows both the actual platooning-induced optimality gap \(G\) and the rule-level upper bound \(\widehat G(\delta,P_{\max})\). Each panel corresponds to one \(N\). The semi-transparent surface shows \(\widehat G\), and colored scatter points show actual \(G\); colors represent arrival rate \(\lambda\). Actual \(G\) points use only bound-checkable rows with proven-optimal NP and PP solutions. With the current completed data and no NP recovery, actual \(G\) points are available for \(N=20\), while the \(N=40\) panel shows the corresponding \(\widehat G\) surface pending exact NP references.}
