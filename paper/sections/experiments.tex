...
\begin{figure}[t]
  ...
  \caption{Method-level mean vehicle delay of NP, CHP, and PP as the platooning threshold or vehicle arrival rate increases, using checked exact-delay rows. The left-column panels vary \(\delta\), and the right-column panels vary \(\lambda\), with rows stratified by \(N\). For this presentation figure, PP is averaged over \(P_{\max}\) to show its aggregate delay position relative to NP and CHP.}
  \label{fig:pp-delay}
\end{figure}
