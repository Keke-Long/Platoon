...
\begin{figure}[t]
  ...
  \caption{Three-dimensional bound validation for cases with exact NP and PP solutions. The horizontal axes are the platooning threshold \(\delta\) and maximum platoon size \(P_{\max}\), and the vertical axis shows both the actual platooning-induced optimality gap \(G\) and the rule-level upper bound \(\widehat G(\delta,P_{\max})\). Colors represent \(N\), marker shapes represent arrival rate \(\lambda\), filled markers show actual \(G\), and open markers show \(\widehat G\). Only bound-checkable rows with proven-optimal NP and PP solutions are included; with the current completed data and no NP recovery, this checked subset is available for \(N=20\).}
