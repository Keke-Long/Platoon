...
This section formulates vehicle scheduling at a general conflict area and then introduces platoon-based dimension reduction. The presentation follows the physical order of the problem: vehicles first arrive from mutually conflicting approaches, a passing sequence determines how they use the shared conflict area, and a contiguous platoon partition restricts the admissible passing sequences.
...
\begin{figure}[t]
  \centering
  \IfFileExists{figures/general_conflict_area_platoons.pdf}{%
    \includegraphics[width=0.95\linewidth]{figures/general_conflict_area_platoons.pdf}%
  }{%
    \fbox{\parbox[c][0.22\textheight][c]{0.88\linewidth}{\centering Placeholder for the general conflict-area and contiguous-platoon illustration.}}%
  }
  \caption{Illustration of vehicles approaching a shared conflict area. Vehicles are indexed by approach and FIFO position, and each approach queue may be partitioned into contiguous platoons.}
  \label{fig:general-conflict-area-platoons}
\end{figure}

Figure~\ref{fig:general-conflict-area-platoons} illustrates the indexing and partition structure for two conflicting approaches. The theoretical development and optimization framework are not restricted to two approaches.
...
\end{equation}
These link-level quantities will be used in Section~\ref{sec:theory} to construct the FIFO-indexed optimality-loss bound.
